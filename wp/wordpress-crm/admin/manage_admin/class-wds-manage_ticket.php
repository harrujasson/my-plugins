...
class WDS_Admin_Manage_Ticket  {
    ...
    function emails_ticket(){
        ...
        $param['host'] ='{'.trim($server_details->host_imap).':'.trim($server_details->port).'/'.trim($server_details->security).'/novalidate-cert}INBOX';
        $param['username'] =trim($server_details->username);
        $param['password'] =trim($server_details->password);
