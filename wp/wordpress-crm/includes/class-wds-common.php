...
trait WDS_Common{
   ...
   function email_ticket_generate($email='',$subject='',$content='',$full_name=''){
       global $wpdb;
       $table= $wpdb->prefix."mwpl_tickets";
       $user = $this->get_user_by_info('email',$email);
       if(!empty($user)){
            ...
            $data['email'] = $email;
            $customer_id = $this->create_customer_user($data['first_name'].'-'.$data['last_name'], $data['email'],$data['first_name'],$data['last_name'],'');
       }
        $data['subject'] = $subject;
        $data['category'] = 'Other';
        $data['description'] = $content;
        $data['customer_id'] = $customer_id;
        return $this->insert_record($table,$data,1);
   }
